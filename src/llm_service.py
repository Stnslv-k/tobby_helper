    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "project_name ОБЯЗАТЕЛЕН — никогда не вызывай без него. "
                "Если проект не известен — сначала спроси пользователя."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
_TOOL_SYSTEM = (
    ...
    "15. НИКОГДА не создавай задачу без project_name. Если пользователь не указал проект или непонятно в какой — сначала вызови list_projects, покажи список и спроси пользователя в какой проект добавить задачу. Только после ответа создавай задачу.\n"
    ...
    "13. Перед ответом ВСЕГДА проверяй результат инструмента. Если результат содержит 'not_supported', 'error', 'no supported fields' или 'permission denied' — сообщи пользователю что операция не выполнена и почему. НИКОГДА не пиши что что-то сделано, если инструмент этого не подтвердил явно.\n"
